...
<template>
           ...
           <div v-for="(curativo, index) in itemDetails.curativos" :key="index">
               <section class="curativo">
                   ...
                   <div class="avaliacao-lesao">
                       <div class="coluna-1">
                           <p><strong>Altura:</strong> 12 cm</p>
                           <p><strong>Largura:</strong> 5 cm</p>
                       </div>
                       <div class="coluna-2">
                           <p><strong>Profundidade:</strong> 1 cm</p>
                           <p><strong>Situação:</strong> Fibrinosa</p>
                       </div>
                   </div>
                   ...
                   <div v-if="curativo.fotos.length > 0">
                       <h3>Imagens</h3>
                        <div class="fotos">
                             <div v-for="(foto, index) in curativo.fotos" :key="index" class="foto-item">
 ...
 import { PacienteCurativoRelatorio, criarPacienteCurativoRelatorio, transformarFotosCurativos } from '@/types/relatorios';
 ...
 const itemDetails = ref<PacienteCurativoRelatorio>(criarPacienteCurativoRelatorio());
 const dateFimPaciente = ref<string>('');
 const dataInicioPaciente = ref<string>('');
 
 onMounted(() => {

   const storedData = localStorage.getItem('pacienteCurativoRelatorioPeriodo');
   if (storedData) {
       const parsedData = JSON.parse(storedData || '{}');
       if (Array.isArray(parsedData) && parsedData.length > 0) {
           itemDetails.value = transformarFotosCurativos(parsedData[0]);
   ...
   const storedDataInicio = localStorage.getItem('dataInicioRelatorioPacientePeriodo') || '';
   const storedDataFim = localStorage.getItem('dataFimRelatorioPacientePeriodo') || ''; 

   console.log(storedDataFim)
